...
try:
    ...
    print("""[-] Import Error: {}\n Please ensure that all module requirements
          are properly installed...""".format(e))
...
parser = argparse.ArgumentParser(prog='WAFFLE', description=banner, 
                                 epilog=' [ WAFFLE[#] | WAF-Bypass Testing Suite | anxiety ♥ ] ',
                                 prefix_chars='--')
...
parser.add_argument('--payload', metavar='Payload', type=str,
                    help='[ REQUIRED | Payload to test against WAF filter:' +
                    ' $ python3 WAFFLE.py <--Target URL> <--payload>\n')
args = parser.parse_args()

url, payload = args.url, args.payload
...
class run:
    def main(self, parser):
        parser.clte_headers = clte_headers
...
if __name__ == '__main__':
    run.sql_fuzz(self, url, payload, payloadBytes)
